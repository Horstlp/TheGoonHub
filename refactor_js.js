...
function getFiles(dir, files = []) {
  ...
  for (const file of list) {
    ...
    if (fs.statSync(filePath).isDirectory()) {
      ...
      if (filePath.endsWith('.js') && filePath.includes('js\\')) {
        files.push(filePath);
      }
    }
  }
...
const jsFiles = getFiles(__dirname);
...
const jsReplacements = [
  ...
  ['style="color:red; margin-top: 20px;"', 'class="text-danger mt-4"'],
  ['style="color:var(--muted)"', 'class="text-muted"'],
  ['style="color:var(--muted); font-size: 0.9rem;"', 'class="text-muted text-sm"'],
  ['style="color:var(--muted); font-size:0.8rem; padding:4px 0;"', 'class="text-muted text-xs py-1"'],
  ['style="background:rgba(244,63,94,0.15); border-color:#f43f5e; color:#fb7185"', 'class="badge-danger"'],
  ['style="margin:0; font-size: 1.2rem;"', 'class="m-0 text-lg"'],
  ['style="font-size: 0.8em;"', 'class="text-xs"'],
  ['style="float:right; color: var(--accent-purple); font-size: 0.75rem; border: 1px solid var(--accent-purple); padding: 2px 6px; border-radius: 4px; font-weight: bold;"', 'class="badge-purple-outline float-right"'],
  ['style="color:#f43f5e; font-size: 0.9rem;"', 'class="text-danger text-sm"'],
  ['style="color:white"', 'class="text-white"'],
  ['style="width:76px;height:76px;background:var(--surface);border-radius:10px;border:1px dashed var(--border);position:absolute;top:4px;left:17px;z-index:1;"', 'class="folder-thumbnail-placeholder"'],
  ['style="display:flex; align-items:center; gap: 4px;"', 'class="flex align-center gap-1"'],
  ['style="background:none; border:none; color:var(--muted); cursor:pointer; font-size:1.1rem; padding:0 4px;"', 'class="btn-icon-muted"'],
  ['style="color: ${isSavedInHere ? \'var(--accent-purple)\' : \'var(--muted)\'}; font-weight: bold;"', 'class="${isSavedInHere ? \'text-purple font-bold\' : \'text-muted font-bold\'}"'],
  ['style="color: #666;"', 'class="text-gray"'],
  ['style="text-align: center; padding: 20px;"', 'class="text-center p-4"'],
  ['style="display:flex;align-items:center;gap:4px;background:rgba(0,0,0,0.2);padding:2px 6px;border-radius:4px;"', 'class="legend-item"'],
  ['style="width:10px;height:10px;background:${color};border-radius:2px;"', 'class="legend-color" style="background:${color};"'],
  ['style="width:10px;height:10px;background:#444;border-radius:2px;"', 'class="legend-color-gray"'],
  ['style="padding: 8px;"', 'class="p-2"'],
  ['style="padding: 8px; color: var(--accent-purple);"', 'class="p-2 text-purple"'],
  ['style="padding: 8px; font-weight: bold; color: var(--text);"', 'class="p-2 font-bold text-normal"'],
  ['style="padding: 8px; font-weight: bold; color: var(--accent-blue);"', 'class="p-2 font-bold text-blue"'],
  ['style="color:var(--muted); font-size: 0.9rem; margin-right: 10px;"', 'class="text-muted text-sm mr-2"'],
  ['style="width: 16px; height: 16px; display: inline-block; vertical-align: middle;"', 'class="spinner-sm"'],
  ['style="pointer-events:none;"', 'class="pointer-none"'],
  ['style="text-transform: capitalize;"', 'class="capitalize"']
];

jsFiles.forEach(f => {
  let content = fs.readFileSync(f, 'utf8');
  jsReplacements.forEach(([find, replace]) => {
    content = content.split(find).join(replace);
  });
  
  // also handle single quotes variants
  jsReplacements.forEach(([find, replace]) => {
    const singleFind = find.replace(/"/g, "'");
    const singleReplace = replace.replace(/"/g, "'");
    content = content.split(singleFind).join(singleReplace);
  });
...
const cssToAdd = `
/* ── JS Injected Component Classes ── */
.mt-4 { margin-top: 20px; }
.text-danger { color: #ef4444; }
.text-sm { font-size: 0.9rem; }
.text-xs { font-size: 0.8rem; }
.py-1 { padding: 4px 0; }
.badge-danger { background: rgba(244,63,94,0.15); border: 1px solid #f43f5e; color: #fb7185; }
.m-0 { margin: 0; }
.text-lg { font-size: 1.2rem; }
.badge-purple-outline { color: var(--accent-purple); font-size: 0.75rem; border: 1px solid var(--accent-purple); padding: 2px 6px; border-radius: 4px; font-weight: bold; }
.float-right { float: right; }
.text-white { color: white; }
.folder-thumbnail-placeholder { width:76px;height:76px;background:var(--surface);border-radius:10px;border:1px dashed var(--border);position:absolute;top:4px;left:17px;z-index:1; }
.flex { display: flex; }
.btn-icon-muted { background:none; border:none; color:var(--muted); cursor:pointer; font-size:1.1rem; padding:0 4px; }
.text-purple { color: var(--accent-purple); }
.text-gray { color: #666; }
.legend-item { display:flex;align-items:center;gap:4px;background:rgba(0,0,0,0.2);padding:2px 6px;border-radius:4px; }
.legend-color { width:10px;height:10px;border-radius:2px; }
.legend-color-gray { width:10px;height:10px;background:#444;border-radius:2px; }
.p-2 { padding: 8px; }
.text-blue { color: var(--accent-blue); }
.mr-2 { margin-right: 10px; }
.spinner-sm { width: 16px; height: 16px; display: inline-block; vertical-align: middle; }
